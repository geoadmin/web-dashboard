...
//  return {"filter":{"fquery":{"query":{"filtered":{"query":{"query_string":{"query":"wmts"}},"filter":{"bool":{"must":[{"range":{"@timestamp":{"from":moment().subtract('seconds', 60),"to":"now"}}}]}}}}}},"size":0};
  return {"filter":{"fquery":{"query":{"filtered":{"query":{"query_string":{"query":"*"}},"filter":{"bool":{"must":[{"range":{"@timestamp":{"from":moment().subtract('seconds', 60),"to":"now"}}},{"fquery":{"query":{"query_string":{"query":"tags:((\"s3-tiles\" AND \"wmts\"))"}},"_cache":true}}]}}}}}},"size":0};
};
