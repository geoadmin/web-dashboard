...
var getURLToday = function() {
  ...
  return 'http'+(config.get('ssl')?'s':'')+'://'+config.get('ES_USERNAME')+':'+config.get('ES_PASSWORD')+'@'+config.get('remote_host')+'/elasticsearch//logstash-'+today+'/_search?search_type=count';
};

var getURLYesterday = function() {
  var yesterday = new moment().subtract('days', 1).format('YYYY.MM.DD');
  return 'http'+(config.get('ssl')?'s':'')+'://'+config.get('ES_USERNAME')+':'+config.get('ES_PASSWORD')+'@'+config.get('remote_host')+'/elasticsearch//logstash-'+yesterday+'/_search?search_type=count';
};
...
var getTilesLastNSecondsOptions = function(n){
  ...
  var today = new moment().format('YYYY.MM.DD');
  options.url = 'http'+(config.get('ssl')?'s':'')+'://'+config.get('ES_USERNAME')+':'+config.get('ES_PASSWORD')+'@'+config.get('remote_host')+'/elasticsearch//logstash-'+today+'/_search';
